...
async def handle_group(event: GroupMessageEvent, args: Message = CommandArg()):
    if player := args.extract_plain_text().strip():
        await matcher.finish(bound_handler(event, player))
    if (user := str(event.user_id)) not in data_manager.players.keys():
        await matcher.finish('你还没有绑定白名单，请先绑定后再试。')
    await matcher.finish(F'用户 {event.sender.card}({user}) 已绑定白名单到 {data_manager.players[user]} 玩家。')


def bound_handler(event: GroupMessageEvent, player: str):
    if not check_player(player):
        return '此玩家名称非法！玩家名称应只包含字母、数字、下划线且长度不超过 16 个字符。'
    if (user := str(event.user_id)) in data_manager.players:
