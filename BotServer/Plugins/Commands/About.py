...
matcher = on_command(
    'about', force_whitespace=True,
    rule=Rules.command_rule, aliases=config.command_aliases.get('about', {})
)
...
def about_handler():
    yield F'当前版本为 {version_manager.version}，{"已是最新版本！" if version_manager.check_update() else "发现新版本，请及时更新！"}'
